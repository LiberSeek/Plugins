...
飞书多维表 FaaS 字段捷径插件，用 BOFT 图像模型为每行记录生成图片，并把结果写回附件字段。
...
- 默认 API：`https://api.boft.ai/v1/images/generations`。
...
- 授权：飞书托管 `HeaderBearerToken`，授权 ID 为 `boft_auth`，平台槽为 `connect_ai`。
...
然后把 `config.json` 中的占位 token 改成自己的 BOFT Key：

```json
{
  "authorizations": [
    "sk-boft-local-debug-token"
  ]
}
```
...
- 确认飞书字段捷径后台允许 `connect_ai` 承载 BOFT API Key。
- 确认 BOFT 正式域名仍为 `api.boft.ai`；如变更，需要同步更新 `basekit.addDomainList` 和 `src/constants.ts`。
- `自定义 Base URL` 只能使用已经写入 `basekit.addDomainList` 的域名；如果用户填写了新的域名，需要先把该域名加入白名单后重新编译发布。
